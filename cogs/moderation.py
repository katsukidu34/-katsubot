"""
cogs/moderation.py — Commandes de modération (JSON only)
"""
...
from config import get_config, ajouter_sanction, get_sanctions, get_warnings, increment_warning
...
COULEURS_ACTIONS = {
    ...
    "Warn": discord.Color.yellow(), "Unban": discord.Color.green(),
...
class Moderation(commands.Cog):
    ...
    async def log_mod(self, guild, action, moderateur, cible, raison):
    ...
    @app_commands.describe(user_id="L'ID Discord du membre")
    @app_commands.checks.has_permissions(ban_members=True)
    ...
    async def warn(self, interaction: discord.Interaction, membre: discord.Member, raison: str = "Aucune raison donnée"):
        await interaction.response.defer()
        nb = increment_warning(interaction.guild_id, membre.id)
        ...
        if nb >= 3:
            try:
                ...
                ajouter_sanction(interaction.guild_id, membre.id, "Kick auto", self.bot.user.id, "Bot", "3 avertissements atteints")
                await self.log_mod(interaction.guild, "Kick auto", interaction.user, membre, "3 avertissements atteints")
                await interaction.channel.send(f"🤖 {membre.mention} expulsé automatiquement après **3 avertissements**.")
            except discord.Forbidden:
    ...
    async def modlog(self, interaction: discord.Interaction, membre: discord.Member):
        ...
        warnings  = get_warnings(interaction.guild_id, membre.id)
        embed = discord.Embed(title=f"📋 Historique — {membre.display_name}", color=discord.Color.blurple(), timestamp=discord.utils.utcnow())
        embed.set_thumbnail(url=membre.display_avatar.url)
        if not sanctions:
        ...
        embed.set_footer(text=f"{len(sanctions)} sanction(s) · {warnings} warn(s)")
    ...
    async def clear(self, interaction: discord.Interaction, nombre: int):
        ...
        await interaction.followup.send(f"✅ **{len(supprimes)} messages** supprimés !", ephemeral=True)
    ...
    @app_commands.command(name="slowmode", description="Active/désactive le slowmode")
    ...
    async def slowmode(self, interaction: discord.Interaction, secondes: int = 0):
        ...
        msg = "✅ Slowmode **désactivé**." if secondes == 0 else f"✅ Slowmode réglé à **{secondes}s**."
        await interaction.followup.send(msg)
